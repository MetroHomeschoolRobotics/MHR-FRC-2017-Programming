	...
	// the robot is enabled in teleop mode)
	{
		...
		// End of code for controller values
				...
				SmartDashboard::PutNumber("Right drive RPM: ", rightDrive1 && rightDrive2->GetSpeed());*/
					...
					// start lowerIntakeRoller code
					if (driveStick->GetRawButton(2)) // pwm 1
					{
						lowerIntakeRoller->Set(-1); // test at 1 or 100 percent
					}
					...
					// combined new intake code

					/*if (driveStick->GetRawButton(2)) // intake
					{
						intake->Set(1);
					}

					else if (driveStick->GetRawButton(4)) // lower intake
					{
						lowerIntakeRoller->Set(1);
					}
					else
					...
					else if (driveStick->GetRawButton(1)) // lower intake
					{
						lowerIntakeRoller->Set(-1);
					}
					else
