...
//	bool on = true;
	...
	void AutonomousPiriodic(){
		...
		if(action == 2) {
			...
			if(COGX != -1) {
				if(turnDistance > 1.5) {
					frontDrive1->Set(-0.45);
					frontDrive2->Set(-0.45);
					rearDrive1->Set(-0.45);
					rearDrive2->Set(-0.45);
	...
	// the robot is enabled in teleop mode)
	{
		...
		while (on) {
			if (limitSwitchState == true) {	//if the limit switch is pressed...
				count++;		//...add one "count" for every press...
				Wait(1);		//...and count the presses for 1 second
				if (count > 48) {		//...but if the "count" is over 48...
					shooter = shooter - 3;//...subtract 3 from the integer that the shooter is "powered" from...
					count = 0;		//...and set the counter to 0
				} else if (count < 48) {//...but if the "count" is less than 48...
