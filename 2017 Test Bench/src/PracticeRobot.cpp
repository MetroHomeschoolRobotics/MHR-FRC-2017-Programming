...
class Robot: public IterativeRobot
{
	...
	private:
		...
		double topShooterSpeed = 0;
		double bottomShooterSpeed = 0;
		...
		{
			topShooterSpeed = shooterTop->GetSpeed();
			bottomShooterSpeed = shooterBottom->GetSpeed();
			...
			{
				if (topShooterSpeed > -410)
				{
					shooterTop->Set(+1);
				}
				else if (topShooterSpeed < -390)
				{
					shooterBottom->Set(-1);
				}

				if (bottomShooterSpeed > 410)
				{
					shooterTop->Set(+1);
				}
				else if (bottomShooterSpeed < 390)
				{
					shooterBottom->Set(-1);
				}
			}
			...
			// start agitator code
			if (driveStick->GetRawButton(5)) // pwm 1
			{
